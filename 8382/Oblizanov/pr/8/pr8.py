...
epochs = input("Enter epochs: ").split(' ')
...
class CustomCallback(keras.callbacks.Callback):
  ...
  def on_epoch_end(self, epoch, logs=None):
    if epoch in epochs:
      ...
      data = [0] * len(set(y_train))
      length = [0] * len(set(y_train))
      for i in range(len(prediction)):
        data[y_train[i]] += float(prediction[i] == y_train[i])
        length[y_train[i]] += 1.0
      for i in range(len(data)):
        data[i] /= length[i]
      x = []
      for i in range(len(data)):
        x.append("Class " + str(i))
      plt.bar(x, data, 0.3)
      plt.xlabel('Classes')
      plt.ylabel('Accuracy')
...
H = model.fit(train_data, train_labels, epochs=12, batch_size=16,
              validation_split = 0.2, callbacks=[CustomCallback(train_data, train_labels)])
