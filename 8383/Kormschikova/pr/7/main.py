...
plt.plot(range(len(loss)),loss, "g", label="loss")
plt.plot(range(len(val_loss)),val_loss,  "b", label="val_loss")
plt.legend()
plt.show()

predicted_res = model.predict(test_data)
pred_length = range(len(predicted_res))
plt.plot(pred_length,test_res, "b", label="sequences")
plt.plot(pred_length, predicted_res, "g", label="model predict")
plt.legend()
plt.show()