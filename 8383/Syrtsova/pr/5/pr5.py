...
data = np.array([-X**3, np.log(np.abs(X)), np.exp(X), X+4, -X+np.sqrt(np.abs(X)), X])
data += e
...
hidden1 = Dense(100, activation='relu')(hidden1)
regression = Dense(1, activation='tanh')(hidden)
regression = Dense(1, activation='linear', name='regression')(concatenate([regression, hidden1]))
autoencoder = Model(input_layer, decoded)
encoder = Model(input_layer, encoded)
encoded_input = Input(shape=(encoding_dim,))
decoder_layer = autoencoder.layers[-1]
regression_model = Model(input_layer, (regression, decoded))
regression_model.compile(optimizer='adam', loss='mse', metrics='mae')
n_epochs = 500 
history = regression_model.fit(x_train, [target_train, x_train], epochs=n_epochs, batch_size=128, shuffle=True, validation_data=(x_test, [target_test, x_test]))
...
plt.show()
plt.plot(x, history.history['regression_loss'])
plt.plot(x, history.history['val_regression_loss'])
...
rm = Model(input_layer, regression)
rm.save('regression.h5')
y = rm.predict(data)
encoder = Model(input_layer, encoded)
...
regression = Model(input_layer, regression)
target = target.reshape((target.shape[0], 1))
regression_res = regression.predict(data).reshape(target.shape)
df = pd.DataFrame(np.concatenate([regression_res, target], axis=1))
df.to_csv('regression.csv', index=False, header=['predicted', 'target'])
